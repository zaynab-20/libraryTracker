...
const bookstable = require("../models/bookstable");
...
exports.createBook = async (req, res) => {
    try {
        ...
        res.status(201).json({ meassage: `new boook created successfully`, data: data })
...
exports.bulkBook = async (req, res) => {
    try {
        ...
        myBulkBook.map((e) => { return e.id = uuidv4() })

        const bulk = await BooksTable.bulkCreate(myBulkBook, { validate: true })

        res.status(201).json({ message: `multiple books successfully created`, data: bulk })

    } catch (error) {
        res.status(500).json({ message: `error creating books`, error: error.meassage })
...
exports.getAll = async (req, res) => {
    try {
        ...
        res.status(200).json({ message: `kindly find below all books`, "total number of books": allBook.length, data: allBook })
    } catch (error) {
...
exports.getOne = async (req, res) => {
    try {
        ...
        res.status(200).json({ message: `kindly find below user with the above id`, data: oneBook })
    } catch (error) {
...
exports.getByGenre = async (req, res) => {
    ...
    const getAllbooksByGenre = await BooksTable.findAll({where: {genre:genre}});
    res.status(200).json({
