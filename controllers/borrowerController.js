...
const borrowerstable = require("../models/borrowerModel");

exports.createBorrower = async (req, res) => {
  try {
    const { BookID } = req.params;
    const { BorrowerName, BorrowDate, ReturnDate, PhoneNumber } = req.body;
    ...
    res
      .status(500)
      .json({ message: "Error creating borrow record", error: error.message });
  }
};
...
exports.bulkBorrow = async (req, res) => {
  try {
    ...
    const bulk = await BorrowersTable.bulkCreate(myBulkBorrow, {
      validate: true,
    });

    res
      .status(201)
      .json({ message: `multiple borrower successfully created`, data: bulk });
  } catch (error) {
    res
      .status(500)
      .json({ message: `error creating borrower`, error: error.meassage });
  }
};
...
exports.getAllBorrowers = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({
        message: `kindly find all the borrowers name`,
        data: myBorrowers,
      });
  } catch (error) {
    res
      .status(500)
      .json({ message: "Error fetching borrow records", error: error.message });
  }
};
...
exports.updateBorrower = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "borrower updated", data: newUpdateBorrower });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
};
...
exports.deleteBorrower = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: `borrower deleted successfully`, data: booksBorrower });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
};
